...
class TuningHybridGB:
    ...
    def cv_tune_results_multi_target_one_top_save_csv(self,
        ...
        for target in targets:
            ...
            x_morse_reduced = h.dim_reduction(x_dict[target], original_top,
                                top,
                                n_directions,
            ...
            x_morse_q, integer_indices = h.summary(x_morse_reduced,
                                           top, 
                                           n_directions,
            ...
            print(x_morse_q.shape)

            # Dimensionally reduce chemical baseline
            x_base = h.baseline_dim_reduction(x_dict_baseline[target],
                                      n_original_properties=n_original_properties, 
                                      desired_properties=desired_properties,
                                      num_quantiles=num_quantiles)
            x_base_q = x_base.to_numpy()
            print(x_base_q.shape)

            # Concatenate features
            # stack along the column axis
            x = np.concatenate((x_morse_q, x_base_q), axis=1)
            print(x.shape)
    ...
    def cv_tune_results_one_target(self,
        ...
        for fold, (train_ids, test_ids) in enumerate(outer_cv):
            ...
            # Add tuned parameters to parameter dictionary 
            print(params)
            ...
            train_data = lgb.Dataset(X[train_ids], 
                                      label=train_labels,
                                      categorical_feature=[],
                                      params={'max_bin': tuned_params['max_bin']})
    ...
    def train_gb(
        ...
        for fold, (train_ids, test_ids) in enumerate(outer_cv):
            ...
            train_data = lgb.Dataset(x_train[train_ids], 
                                      label=train_labels,
                                      categorical_feature=[],
                                      params={'max_bin': trial_params['max_bin']})
