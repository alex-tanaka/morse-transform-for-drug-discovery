...
class MoleculeComplexAllTop:
    ...
    Uses the top critical and regular vertices.
    ...
    def getCriticalPoints(self,
        ...
        top: False or int
            ...
            vector only contains the top 'top' points for a given
            direction. If there are fewer than 'top' points in a given direction 
            then the feature vector is zero paddded. If both 'top' and 'pad_to_len' 
            are specified then 'top' has priority.
        feature: A boolean
        ...
        # Get the indices that sort the projected points by descending projected height
        ...
        # Keep only the top points
        if ( (top is not False) and isinstance(top, (int, float)) ):
            N_top = int(top)

            # Create an array of the points and their indices
            sorted_points = [row for row in critical_points[2]]
            ...
            if L == 0:
                ...
                feature_matrix = sorted_points[:N_top]
            elif L < N_top:
                feature_matrix[0:L, :] = sorted_points
