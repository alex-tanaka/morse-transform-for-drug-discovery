...
class MoleculeComplex:
    ...
    def plot_skeleton(self, n=2, renderer='colab'):
        ...
        if n not in [0, 1, 2]:
            raise TypeError('n must be 0, 1 or 2.')
    ...
    def getCriticalPoints(self,
        ...
        # Get the indices that sort the projected points by descending height t
        ...
        # A counter for determining when to end the following loop if a top value
        # is chosen
        N_critical_points = 0
        ...
        for idx, vertex in enumerate(X_sorted_indices):
            ...
            # If the 0-simplex is critical then increment the counter
            if np.sum(reduced_betti_numbers) >= 1:
                N_critical_points += 1

            # Exit the loop if the top number of critical points has been found
            if top and (N_critical_points == top):
                break

        # Keep only the top critical points
        if ( (top is not False) and isinstance(top, (int, float)) ):
            N_top = int(top)

            # Create an array of only the critical points.
            # If a 0-simplex has at least one reduced Betti number greater 
            # than zero, then it is critical
            only_critical_points = [row 
                                    for row in critical_points[2]
                                    if np.sum(row[1:4]) >= 1]
            only_critical_points = np.array(only_critical_points)

            # Keep only the first 'N_top' critical points, when ordered by
            # descending projected height (t-ordinate). If there are fewer than
            # 'N_top' critical points, then zero pad the matrix.
            ...
            if L == 0:
                ...
                feature_matrix = only_critical_points[:N_top]
            elif L < N_top:
                feature_matrix[0:L, :] = only_critical_points
        ...
        df.rename(columns={'symbol': 'critical index'}, inplace=True)
